...
/**
 * إعدادات مصدر «قرمزي ORG» (krmzi). تُعرض من زر الإعدادات في CloudStream.
 *
 * كل خيار يُخزَّن في ملف التفضيلات نفسه الذي يقرأه [KrmziProvider] مباشرة
 ...
 */
class KrmziSettingsBottomSheet(private val prefs: SharedPreferences) : BottomSheetDialogFragment() {
    ...
    companion object {
        // ★ اسم ملف التفضيلات — يجب أن يطابق ما يمرّره KrmziPlugin للمصدر،
        //   وإلا ذهبت الاختيارات إلى ملف آخر لا يقرأه (سلوك صامت بلا أثر).
        const val PREFS_NAME = "Krmzi"

        // مفاتيح هذه الوحدة (سلسلة نصية فريدة—no تعارض مع أي وحدة أخرى).
        const val KEY_QUALITY_ORDER = "kr_quality_order"     // "default" | "asc" | "desc"
        const val KEY_SHOW_RAW_LINK = "kr_show_raw_link"     // Boolean — الافتراضي true

        fun show(fm: FragmentManager, prefs: SharedPreferences) {
            KrmziSettingsBottomSheet(prefs).show(fm, "kr_settings")
        }
    }
    ...
    class PrefsFragment(private val prefs: SharedPreferences) : PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            // عند إيقافه يُسقَط فرعا «صفحة التضمين كـ VIDEO» (يُعرضان حين لا يُعثر
            // على أي HLS/MP4 داخلها) وتبقى روابط الوسائط الحقيقية كما هي.

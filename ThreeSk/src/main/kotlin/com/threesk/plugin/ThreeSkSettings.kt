...
/**
 * إعدادات مصدر «قصة عشق» (ThreeSk). تُعرض من زر الإعدادات في CloudStream.
 *
 * كل خيار يُخزَّن في ملف التفضيلات نفسه الذي يقرأه [ThreeSk] مباشرة
 ...
 * و«كل الخوادم» يعرض كل روابط الخوادم كما تُنتج اليوم.
 */
class ThreeSkSettingsBottomSheet(private val prefs: SharedPreferences) : BottomSheetDialogFragment() {
    ...
    companion object {
        // ★ اسم ملف التفضيلات — يجب أن يطابق ما يمرّره ThreeSkPlugin للمصدر،
        //   وإلا ذهبت الاختيارات إلى ملف آخر لا يقرأه (سلوك صامت بلا أثر).
        const val PREFS_NAME = "ThreeSk"

        // مفاتيح هذه الوحدة (سلسلة نصية فريدة—no تعارض مع أي وحدة أخرى).
        const val KEY_QUALITY_ORDER = "3sk_quality_order"   // "default" | "asc" | "desc"
        const val KEY_MAX_SERVERS = "3sk_max_servers"       // "all" | "3" | "5"

        fun show(fm: FragmentManager, prefs: SharedPreferences) {
            ThreeSkSettingsBottomSheet(prefs).show(fm, "3sk_settings")
        }
    }
    ...
    class PrefsFragment(private val prefs: SharedPreferences) : PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            // ترتيب الجودات عند البث — يعيد ترتيب روابط الخوادم فقط بلا حذف
            ...
            // عدد الخوادم المعروضة — الافتراضي «كل الخوادم» = كل روابط الخوادم
            // كما تُنتج اليوم بلا نقص.
            val maxServersPref = ListPreference(ctx).apply {
                key = KEY_MAX_SERVERS
                title = "عدد الخوادم المعروضة"
                summary = "افتراضي: كل الخوادم (كما هي)"
                summaryProvider = ListPreference.SimpleSummaryProvider.getInstance()
            ...
            playbackCategory.addPreference(maxServersPref)

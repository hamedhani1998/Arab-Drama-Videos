...
/**
 * إعدادات مصدر «NetShort». تُعرض من زر الإعدادات في CloudStream.
 *
 * كل خيار يُخزَّن في ملف التفضيلات نفسه الذي يقرأه [NetShortProvider] مباشرة
 ...
 * (المصدر بث رابط واحد 720p أصلاً فلا يوجد ما يُرتَّب)، وإظهار الترجمة
 * مفعّل كما هو.
 */
class NetShortSettingsBottomSheet(private val prefs: SharedPreferences) : BottomSheetDialogFragment() {
    ...
    companion object {
        // ★ اسم ملف التفضيلات — يجب أن يطابق ما يمرّره NetShortPlugin للمصدر،
        //   وإلا ذهبت الاختيارات إلى ملف آخر لا يقرأه (سلوك صامت بلا أثر).
        const val PREFS_NAME = "NetShort"

        // مفاتيح هذه الوحدة (سلسلة نصية فريدة—no تعارض مع أي وحدة أخرى).
        const val KEY_QUALITY_ORDER = "ns_quality_order"        // "default" | "asc" | "desc"
        const val KEY_SHOW_SUBTITLES = "ns_show_subtitles"     // Boolean — الافتراضي true

        fun show(fm: FragmentManager, prefs: SharedPreferences) {
            NetShortSettingsBottomSheet(prefs).show(fm, "ns_settings")
        }
    }

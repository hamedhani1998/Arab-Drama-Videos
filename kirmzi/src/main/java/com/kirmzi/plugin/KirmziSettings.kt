...
/**
 * إعدادات مصدر «قرمزي TV» (kirmzi). تُعرض من زر الإعدادات في CloudStream.
 *
 * كل خيار يُخزَّن في ملف التفضيلات نفسه الذي يقرأه [KirmziProvider] مباشرة
 ...
 * وإظهار رابط الصفحة الخام مفعّل كما هو.
 */
class KirmziSettingsBottomSheet(private val prefs: SharedPreferences) : BottomSheetDialogFragment() {
    ...
    companion object {
        // ★ اسم ملف التفضيلات — يجب أن يطابق ما يمرّره KirmziPlugin للمصدر،
        //   وإلا ذهبت الاختيارات إلى ملف آخر لا يقرأه (سلوك صامت بلا أثر).
        const val PREFS_NAME = "Kirmzi"

        // مفاتيح هذه الوحدة (سلسلة نصية فريدة—no تعارض مع أي وحدة أخرى).
        const val KEY_QUALITY_ORDER = "km_quality_order"     // "default" | "asc" | "desc"
        const val KEY_SHOW_RAW_LINK = "km_show_raw_link"     // Boolean — الافتراضي true

        fun show(fm: FragmentManager, prefs: SharedPreferences) {
            KirmziSettingsBottomSheet(prefs).show(fm, "km_settings")
        }
    }
    ...
    class PrefsFragment(private val prefs: SharedPreferences) : PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            // ترتيب الجودات عند البث — يعيد ترتيب روابط السيرفرات فقط بلا حذف
            // ولا تكرار؛ المتساوية تحتفظ بترتيبها، والافتراضي = نفس ترتيب اليوم
            // حرفياً. (كل روابط هذا المصدر بجودة Unknown، لذا قد يبدو الاختيار
            // بلا أثر بصري — وهذا صحيح ومتوقّع.)
            ...
            // إظهار رابط الصفحة الخام كبديل — الافتراضي مفعّل، أي سلوك اليوم حرفياً.
            // عند إيقافه يُسقَط فرع «صفحة التضمين كـ VIDEO» (يُعرض حين لا يُعثر على
            // أي HLS/MP4 داخلها) وتبقى روابط الوسائط الحقيقية كما هي.
            val showRawLinkPref = SwitchPreferenceCompat(ctx).apply {
                key = KEY_SHOW_RAW_LINK
                title = "إظهار رابط الصفحة الخام كبديل"
                summary = "عند تعذّر فك السيرفر، يعرض رابط صفحة التضمين بدل «لا روابط»"

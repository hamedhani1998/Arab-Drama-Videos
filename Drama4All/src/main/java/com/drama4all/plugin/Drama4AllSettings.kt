 ...
 * مسار ترجمة في روابط البث أصلاً — لا داعي لخيار يمرّر null بلا أثر.
 */
class Drama4AllSettingsBottomSheet(private val prefs: SharedPreferences) : BottomSheetDialogFragment() {
    ...
    companion object {
        // ★ اسم ملف التفضيلات — يجب أن يطابق ما يمرّره Drama4AllPlugin للمصدر،
        //   وإلا ذهبت الاختيارات إلى ملف آخر لا يقرأه (سلوك صامت بلا أثر).
        const val PREFS_NAME = "Drama4All"

        // مفاتيح هذه الوحدة (سلسلة نصية فريدة—no تعارض مع أي وحدة أخرى).
        const val KEY_QUALITY_ORDER = "d4a_quality_order"        // "default" | "asc" | "desc"

        fun show(fm: FragmentManager, prefs: SharedPreferences) {
            Drama4AllSettingsBottomSheet(prefs).show(fm, "d4a_settings")
        }
    }
    ...
    class PrefsFragment(private val prefs: SharedPreferences) : PreferenceFragmentCompat() {
        ...
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            ...
            // ترتيب الجودات عند البث — المصدر يبث رابطاً واحداً فقط لكل حلقة
            // (ملف mp4 أو master أو ملف media — واحد منهما فقط)، فأي ترتيب = نفس
            ...
            val qualityOrderPref = ListPreference(ctx).apply {
                ...
                title = "ترتيب الجودات"
                summary = "المصدر يبث رابطاً واحداً لكل حلقة — الترتيب لا يغيّر شيئاً"
